...
{
  public function __construct($periode,$triwulan,$rekap,$data_renja)
    {
  ...
  public function view(): View
  {
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class    => function(AfterSheet $event) {
              ...
              //set kolom akhir
                ...
                //Page Setup: Scaling options
                  ...
                  $opd=Data_Opd::orderBy('id')->get();
                  $prog_rpjmd=Rpjmd_Prog::all();
                  if($data_renja=="awal"){
                    $renja=Renja::where('periode',$periode)->where('bappeda',1)->get();                    
                  }elseif($data_renja=="perubahan"){
                    $renja=Renja_Per::where('periode',$periode)->where('bappeda',1)->get();
                  }

                  // $renja=Renja::all();

                  $hjmlurusan="=H".($baris_akhir+1);
                  $jjmlurusan="=J".($baris_akhir+1);
                  $ljmlurusan="=L".($baris_akhir+1);
                  
                  $njmlurusan="=N".($baris_akhir+1);
                  $pjmlurusan="=P".($baris_akhir+1);
                  $rjmlurusan="=R".($baris_akhir+1);
                  $tjmlurusan="=T".($baris_akhir+1);
                  $baristot_urusan=$baris_akhir;
				  
				  //mulai non urusan
				  foreach ($nonurusan as $keyurusan => $vurusan) {
                    if($vurusan->unitkey!="225_"){
                        $du = Dafunit::where('unitkey','=',$vurusan->unitkey)->first();
                        $nonurusan = $du->nm_unit;
                        $unitkey=$vurusan->unitkey;
                    }else{
                        $du = Dafunit::where('unitkey','=','212_')->first();
                        $sekda = $opd->where('unit_key','=','80_')->first();
                        $nonurusan = $du->nm_unit.' : '.$sekda->nm_instansi;   
                        $unitkey='80_';           
                    }
                    ...
                    $event->sheet->getDelegate()->setCellValue('C'.$baris_akhir,$vurusan->kdunit);
                      $event->sheet->getDelegate()->setCellValue('D'.$baris_akhir,$nonurusan);
          ...
          // $opdnonurusan=$vurusan->opd_rkpd($periode,$unitkey,$data_renja)->whereNotIn('id_instansi',[7,8,9,10]);
					$opdnonurusan=$vurusan->opd_rkpd($periode,$unitkey,$data_renja)->whereNotIn('id_instansi',[7,8,9,10]);
                      foreach ($opdnonurusan as $keyopd => $vopd) {
                        $baris_akhir++;
                        ...
                        $event->sheet->getDelegate()->getStyle('A'.$baris_akhir.':'.$kolom_akhir.''.$baris_akhir)->applyFromArray($style_opd);
                          ...
                          foreach ($vopd->program_rkpd($periode,$unitkey,$vopd->id_instansi,'nonurusan') as $keyprog => $vprog) {
                            $no_keg=0;
                            $baris_akhir++;
                            ...
                            $event->sheet->getDelegate()->getStyle('A'.$baris_akhir.':'.$kolom_akhir.''.$baris_akhir)->applyFromArray($styleprogram);
                                ...
                                $baris_prog=$baris_akhir;
                                  ...
                                  // start kegiatan
                                  $kegiatan=$renja->where('id_instansi',$vopd->id_instansi)->where('idprgrm',$vprog->idprgrm);
                                  $no=0;
                  ...
                  foreach ($urusan as $keyurusan => $vurusan) {
                    ...
                    $baris_urusan=$baris_akhir;
                      foreach ($vurusan->opd_rkpd($periode,$unitkey,$data_renja) as $keyopd => $vopd) {
                        $baris_akhir++;
                        ...
                        $event->sheet->getDelegate()->getStyle('A'.$baris_akhir.':'.$kolom_akhir.''.$baris_akhir)->applyFromArray($style_opd);
                          ...
                          $rjmlprog="=R".($baris_akhir+1);
                          $tjmlprog="=T".($baris_akhir+1);
                          foreach ($vopd->program_rkpd($periode,$unitkey,$vopd->id_instansi,'urusan') as $keyprog => $vprog) {
                            $no_keg=0;
                            $baris_akhir++;
                  ...
                  //wrap text
                  $event->sheet->getDelegate()->getStyle('B8:F'.$baris_akhir)->getAlignment()->setWrapText(true);
