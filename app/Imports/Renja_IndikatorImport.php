...
class Renja_IndikatorImport implements ToModel,WithStartRow
{
    ...
    */
    ...
    {
        $renja=Renja::where('id_instansi',$row[0])->where('kdkegunit',$row[1])->where('periode',$this->id_periode_renja)->first();
        if($renja!=""){
            $renja_indikator=Renja_Indikator::where('id_renja',$renja->id);
            $id_renja=$renja->id;
