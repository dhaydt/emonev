                    ...
                    <table width="100%" class="table table-bordered">
                        ...
                        <tr>
                            <td>
                                ...
                                @endphp
                                <input type="radio" name='st1' idi="{{$v->id_instansi}}" value="0" {{$cs_t1}} class="ste55" {{$dist1}}>Belum
                                <input type="radio" name='st1' idi="{{$v->id_instansi}}" value="1" {{$cs_t12}} class="ste55" {{$dist1}}>Sudah Selesai
                            </td>
                            <td>
                                <input type="radio" name='st2' idi="{{$v->id_instansi}}" value="0" {{$cs_t2}} class="ste55" {{$dist2}}>Belum
                                <input type="radio" name='st2' idi="{{$v->id_instansi}}" value="1" {{$cs_t22}} class="ste55" {{$dist2}}>Sudah Selesai
                            </td>
                            <td>
                                <input type="radio" name='st3' idi="{{$v->id_instansi}}" value="0" {{$cs_t3}} class="ste55" {{$dist3}}>Belum
                                <input type="radio" name='st3' idi="{{$v->id_instansi}}" value="1" {{$cs_t32}} class="ste55" {{$dist3}}>Sudah Selesai
                            </td>
                            <td>
                        ...
                        <div class="panel-body" style="padding:10px;">
                            ...
                            <div class="btn-group">
                                 @if (Auth::guard('web')->check())
                                 <a ref="#" value="{{$v->id_instansi}}" jenis="e19" class="btn btn-danger btn-xs modal_ekspor" data-toggle="modal" data-target="#modal_ekspor" title="{{$v->opd->nm_instansi}}" judul="RKPD" tipe="Excel"><i class="fa fa-book"></i> E.19</a>
                                 @endif
                                 <a ref="#" value="{{$v->id_instansi}}" jenis="e55" class="btn btn-success btn-xs modal_ekspor" data-toggle="modal" data-target="#modal_ekspor" title="{{$v->opd->nm_instansi}}" tipe="Excel" judul="Renja"><i class="fa fa-file-excel-o"></i> Excel</a>
                                 
                                 <a href="#" class="btn btn-warning btn-xs"><i class="fa fa-search"></i> Preview</a>
                             </div><p></p>
                                    ...
                                    <div id="collapse{{$v->id_instansi}}{{$value}}{{$no}}{{$no2}}" class="panel-collapse collapse bg-silver">
                                        <div class="panel-body bg-success">
											...
											<tr>
                                                ...
                                                @php $urno=1; $tot_keg=0; $tot_urusan_pagu=0; $tot_urusan_real=0; @endphp
                                                @foreach($rpjmd_prog_urusan as $vp)
                                                    ...
                                                    <tr>
                                                        <td>{{$urno++}}</td>
                                                        <td>
                                                            <a ref="#" value="{{ action('RenjaController@show_evaluasi_renja',['periode'=>$periode,'triwulan'=>$triwulan,'idprgrm'=>$vp->idprgrm, 'id_instansi'=>$v->id_instansi,'data_renja'=>$data_renja]) }}" class="btn btn-xs modalMd" title="Renja - Program : {{$vp->nmprgrm}}" data-toggle="modal" data-target="#modalMd" style="white-space: pre-wrap;">{{$vp->nmprgrm}}</a>
                                                        </td>
														@php
                                                    ...
                                                    elseif($p_predikat>=51){$predikat="R";}
                                                        ...
                                                        <td align="center">
                                                        	<span class="label label-{{$warna_1}}" style="font-size:12px;">
                                                        		@php
                                                        		if($persen_t1=="100"){
                                                        			echo number_format($persen_t1,0)."%";
	                                                        	}else{
		                                                        	echo number_format($persen_t1,0)."%";
	                                                        	}
	                                                        	@endphp
                                                        ...
                                                        <td align="center">
                                                        	<span class="label label-{{$warna_2}}" style="font-size:12px;">
                                                        	@php
	                                                		if($persen_t2=="100"){
	                                                			echo number_format($persen_t2,0)."%";
	                                                    	}else{
	                                                        	echo number_format($persen_t2,2)."%";
	                                                    	}
	                                                    	@endphp
                                                        	</span>
                                                        </td>
                                                        <td align="center">
                                                        	<span class="label label-{{$warna_3}}" style="font-size:12px;">
                                                        	@php
	                                                		if($persen_t3=="100"){
	                                                			echo number_format($persen_t3,0)."%";
	                                                    	}else{
	                                                        	echo number_format($persen_t3,2)."%";
	                                                    	}
	                                                    	@endphp
                                                        	</span>
                                                        </td>
						...
						@endphp
						<h4 class='alert alert-success'><b>TOTAL PAGU OPD: {{number_format($tot_opd_pagu,0)}} | TOTAL REALISASI OPD: {{number_format($tot_opd_real,0)}} | ({{number_format($p_predikat,2)}}%  - {{$predikat}})</b></h4>
				@if (Auth::guard('web')->check())		
					</div>
  ...
  transform: translate(-50%, -50%);" role="dialog">
    ...
    <script type="text/javascript">
        ...
        $(document).on('ajaxComplete ready', function () {
            $('.modalMd').off('click').on('click', function () {
                $('#modalMdContent').load($(this).attr('value'), function( response, status, xhr ) {
                    ...
                    console.log(response.msg.store);
                  if ( status == "error" ) {
        ...
        $("#btn_ekspor").click(function(e) {
            ...
            url = "{{url('evaluasi_renja_excel/'.$periode)}}/"+jenis+"/"+id_instansi+"/"+triwulan+"/"+rekap+"/"+data_renja;
            window.open(url);
            $('#wait_download').hide();
