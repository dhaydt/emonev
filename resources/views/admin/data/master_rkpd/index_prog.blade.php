...
<div class="row">
    ...
    <div class="panel panel-success">
        ...
        <p><a class="btn btn-danger btn-sm " href="./data-rkpd?periode={{$periode}}&data_renja={{$data_renja}}"><i class="fa fa-arrow-left"></i> Kembali</a> 
            @if(@Auth::user()->level=='Super Admin')
            | <a ref="#" class="btn btn-info btn-sm modal_1"><i class="fa fa-plus"></i> Tambah Program</a>
            @endif
        </p>
        @if($periode!="")
    ...
    <tr>
        ...
        <th rowspan="2">Lihat Kegiatan</th>
    </tr>
    <tr>
        <th>Awal</th>
    ...
    <tr>
        ...
        <td>{{$p->master_program->nomor}}
            <div class="dropdown">
              ...
              <ul class="dropdown-menu" aria-labelledby="dropdownMenu1">
                @if(@Auth::user()->level=='Super Admin')
                <li><button id="{{$p->id}}" kd='{{$p->idprog}}' prog_awal='{{$p->prog_awal}}' prog_perubahan='{{$p->prog_perubahan}}' class="modal_1_edit">Edit</button></li>
                @endif
                <form @if(@Auth::user()->level=='Super Admin') action="{{route('data-rkpd.destroy', $p->id)}}" @endif method="post" >
                    ...
                    <input type="hidden" name="tabel" value="rkpd_prog">
        ...
        @php
        if($p->nmprog!=""){
            $nomenklatur_prog=$p->nmprog;
        }else{
            $nomenklatur_prog=@$p->master_program->nmprgrm;
        }
        @endphp
            {{@$nomenklatur_prog}}
        </td>
        <td>@php if(@$p->master_program->nmprgrm!=""){ echo @$p->master_program->urusan()->nm_unit; } @endphp</td>
        <td align="right">{{number_format($p->pagu($opd,$periode,$p->idprog)->jpagu_awal,0)}}</td>
        @if($data_renja=="perubahan") 
        <td align="right">{{number_format($p->pagu($opd,$periode,$p->idprog)->jpagu_perubahan,0)}}</td>
        @endif
        <td>
            @if(@Auth::user()->level=='Super Admin')
            <a ref="#" class="btn btn-info btn-xs modal_indikator" idprog='{{$p->idprog}}' nmprog='{{@$p->master_program->nmprgrm}}'><i class="fa fa-plus"></i> Tambah Indikator</a>
            @endif
            @foreach($rules as $r)
                @if($r->ind == 1)
                    @if($r->tambah == 1)
            ...
            <table class="table table-bordered">
                ...
                @php $noin=0; @endphp
                @foreach($p->rkpd_prog($data_renja) as $in)
                @php $noin++; @endphp
                <tr>
                	<td>{{$noin}}</td>
                	...
                	<td>
                		<div class="dropdown">
                		  ...
                		  <ul class="dropdown-menu" aria-labelledby="dropdownMenu1">
                		    @if(@Auth::user()->level=='Super Admin')
                		    <li><button id="{{$in->id}}" kd='{{$p->idprog}}' nmprog='{{@$p->master_program->nmprgrm}}' prog_ind_awal='{{$in->prog_ind_awal}}' indikator_awal='{{$in->indikator_awal}}' raw_sat_awal='{{$in->raw_sat_awal}}' sat_awal='{{$in->sat_awal}}' target_awal='{{$in->target_awal}}' indikator_perubahan='{{$in->indikator_perubahan}}' raw_sat_perubahan='{{$in->raw_sat_perubahan}}' sat_perubahan='{{$in->sat_perubahan}}' target_perubahan='{{$in->target_perubahan}}' prog_ind_perubahan='{{$in->prog_ind_perubahan}}' class="modal_1_edit_indikator">Edit</button></li>
        ...
        <td>
            <a href="{{url('./data-rkpd?periode='.$periode.'&data_renja='.$data_renja.'&act=rkpd_kegiatan&opd='.$p->idopd.'&idprog='.$p->idprog)}}" class="btn btn-xs btn-info"> Lihat Kegiatan <i class="fa fa-arrow-right"></i></a></td>
    </tr>
    @endforeach
    </tbody>
    </table>
          ...
          <div class="modal-body">
                ...
                <form class="form-horizontal" method="post" action="{{route('data-rkpd.store')}}">
                  ...
                  <div class="form-group">
                    ...
                    <div class="col-sm-8"> 
                        <select name="idprog" class="form-control" id="sel">
                            <option value="">Pilih Program</option>
                            @foreach($program as $k)
                            <option value="{{$k->id}}">[{{$k->id}}] {{$k->nmprgrm}}</option>
                            @endforeach
                        </select>
                    </div>
                  </div>
                  <div class="form-group">
                    ...
                    <div class="col-sm-5">
                      <input type="checkbox" name="prog_awal" value="1" @if($data_renja=="awal") selected @endif>
                      <label> RKPD Awal </label><br>
                      <input type="checkbox" name="prog_perubahan" value="1"  @if($data_renja=="perubahan") selected @endif>
...
<!--           <div class="modal-footer">
        ...
        <div id="myModal_indikator" class="modal fade" role="dialog">
          <div class="modal-dialog" style="width: 70%;height: auto;margin: auto;">
            ...
            <div class="modal-content">
              <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal">&times;</button>
                <h4 class="modal-title">Indikator Program</h4>
              </div>
              <div class="modal-body">
                    <div><b>[Periode RKPD]</b> : {{ $periode }} <span style="color:red;font-size: 20px"><b>{{$data_renja}}</b></span></div>
                    <div><b>[OPD]</b> : {{ $data_opd->nm_instansi }}</div>
                    ...
                    <form class="form-horizontal" method="post" action="{{route('data-rkpd.store')}}">
                      ...
                      <div class="form-group">
                        <label class="control-label col-sm-2">Data RKPD:</label>
                        <div class="col-sm-5">
                          <input type="checkbox" name="prog_ind_awal" value="1" @if($data_renja=="awal") checked @endif>
                          <label> RKPD Awal </label><br>
                          <input type="checkbox" name="prog_ind_perubahan" value="1" @if($data_renja=="perubahan") checked @endif>
                          <label> RKPD Perubahan </label><br>
                        </div>
                      </div>
                      ...
                      <div class="form-group">
                        ...
                        <div class="col-sm-3">
                            <input type="text" id="target-ind-awal" class="form-control" name="target_awal" placeholder="target awal">
                        </div>
                        <label class="control-label col-sm-2">Satuan Awal:</label>
                        <div class="col-sm-3">
                            <input type="text" id="sat-ind-awal" class="form-control" name="sat_awal" placeholder="satuan awal">
                        </div>
                      ...
                      <input type="hidden" name="act" value="{{$act}}">
                      <input type="hidden" name="opd" value="{{$opd}}">
                      <input type="hidden" name="idprog" id="idprog">
    ...
    <script type="text/javascript">
        $('.modal_1').click(function(){
            ...
            @if($data_renja=="perubahan")
            $("input[name='prog_awal']").prop('checked', false);
            $("input[name='prog_perubahan']").prop('checked', true);
            @else
            $("input[name='prog_awal']").prop('checked', true);
            $("input[name='prog_perubahan']").prop('checked', false);
            @endif
        });
        $('.modal_1_edit').click(function(){
            ...
            if($(this).attr('prog_awal')=="1"){
                $("input[name='prog_awal']").prop('checked', true);
            }else{
                $("input[name='prog_awal']").prop('checked', false);
            }
            if($(this).attr('prog_perubahan')=="1"){
                $("input[name='prog_perubahan']").prop('checked', true);
            }else{
                $("input[name='prog_perubahan']").prop('checked', false);
            }
        ...
        $('.modal_indikator').click(function(){
            ...
            $(".tambah_ind").val('Simpan Indikator');
            $('.program-title').text($(this).attr('nmprog'));
            $(".modal-title").text('Tambah Indikator Program');
            $("#idprog").val($(this).attr('idprog'));
            ...
            @if($data_renja=="perubahan")
            $("input[name='prog_ind_awal']").prop('checked', false);
            $("input[name='prog_ind_perubahan']").prop('checked', true);
            @else
            $("input[name='prog_ind_awal']").prop('checked', true);
            $("input[name='prog_ind_perubahan']").prop('checked', false);
            @endif
        ...
        $('.modal_1_edit_indikator').click(function(){
        	...
        	$(".tambah_ind").val('Edit Indikator');

        	$('.program-title').text($(this).attr('nmprog'));
        	$(".modal-title").text('Edit Indikator Program');
        	$("#idprog").val($(this).attr('kd'));
        	...
        	if($(this).attr('prog_ind_awal')=="1"){
        	    $("input[name='prog_ind_awal']").prop('checked', true);
        	}else{
        	    $("input[name='prog_ind_awal']").prop('checked', false);
        	}
        	if($(this).attr('prog_ind_perubahan')=="1"){
        	    $("input[name='prog_ind_perubahan']").prop('checked', true);
        	}else{
        	    $("input[name='prog_ind_perubahan']").prop('checked', false);
        	}
